...
public class WorkstationServiceImpl implements IWorkstationService {
    ...
    public int deleteWorkstationById(Integer id) {
         ...
         */
        if (work.getDevId() > 0) {
            devListMapper.updateJsDevListInfo(work.getDevId(), null, user.getCompanyId(), DevConstants.DEV_SIGN_NOT_USE, null);
        }
        if (work.getcId() > 0) {
            devListMapper.updateJsDevListInfo(work.getcId(), null, user.getCompanyId(), DevConstants.DEV_SIGN_NOT_USE, null);
        }
        if (work.geteId() > 0) {
            devListMapper.updateJsDevListInfo(work.geteId(), null, user.getCompanyId(), DevConstants.DEV_SIGN_NOT_USE, null);
        }
        return row;
    }
